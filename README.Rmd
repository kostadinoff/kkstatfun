...
knitr::opts_chunk$set(
  ...
  message = FALSE,
  warning = FALSE,
  # AGG renders the Roboto Condensed set below without showtext.
  dev = if (requireNamespace('ragg', quietly = TRUE)) 'ragg_png' else 'png'
)
options(width = 90, tibble.print_min = 8, tibble.print_max = 20)
set.seed(1)
pkgload::load_all('.', quiet = TRUE)
# kkstatfun no longer attaches these for you (they moved from Depends to
# Imports in 1.1.0), so the examples below attach what they use.
suppressMessages({
  library(dplyr); library(ggplot2); library(tibble); library(survival)
})
# Roboto Condensed resolves via systemfonts, so the AGG/ragg device renders it
# directly -- no showtext needed.
suppressWarnings(suppressMessages(
  set_plot_font(font = 'Roboto Condensed', size = 12)
))
```
...
```{r ex-genpal-reset, include=FALSE}
# restore the flag anchors for the rest of the README
