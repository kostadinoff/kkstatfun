...
## Quasi-Experimental Designs (`R/did.R`, `R/rdd.R`, `R/its.R`, `R/synth.R`, `R/iv.R`)

Designs that identify a causal effect from how the intervention was assigned — a threshold rule, a difference in timing, an instrument, a donor pool — rather than from adjustment for measured confounders. Each function returns the diagnostic for its own identifying assumption. All take individual-level or long panel data as the first argument, honour `dplyr::group_by()`, and return tibbles.

### `kk_did(data, outcome, treated, post, covariates = NULL, unit = NULL, time = NULL, cluster = NULL, family = "gaussian", offset = NULL, conf.level = 0.95)`
Difference-in-differences. The canonical 2x2 estimand is the `treated` x `post` interaction: the change in the treated group net of the change in the control group. Supplying `unit` and `time` fits the two-way fixed-effects version instead and clusters the standard errors on `unit` by default. `family = "binomial"`/`"poisson"` estimates on the link scale and also reports the exponentiated ratio-of-ratios. Returns a one-row tibble with `estimate`, `std.error`, CI, `p.value`, `n_clusters` and `unadjusted` (the raw 2x2 difference of cell means); the four cell means are attached as attribute `cells` and the fitted model is available via `kk_model()`. Under staggered adoption use `kk_did_staggered()` instead.

### `kk_event_study(data, outcome, unit, time, treat_time, covariates = NULL, reference = -1, leads = NULL, lags = NULL, cluster = NULL, conf.level = 0.95)`
Dynamic difference-in-differences: one coefficient per period relative to adoption, from a two-way fixed-effects regression on lead and lag indicators, with `reference` (default the last pre-treatment period) omitted as the baseline. Never-treated units are marked by `NA`/`Inf` in `treat_time` and act as pure controls. The leads are the parallel-trends check. `leads`/`lags` bin the endpoints. Returns a tibble with `rel_time`, `period` (`"pre"`/`"ref"`/`"post"`), `estimate`, `std.error`, CI and `p.value`; the joint Wald test over all leads is attached as attribute `pretrend`.

### `kk_did_staggered(data, outcome, unit, time, treat_time, covariates = NULL, control = c("notyettreated","nevertreated"), base_period = c("universal","varying"), n_boot = 200, conf.level = 0.95)`
Callaway-Sant'Anna group-time average treatment effects for staggered adoption, where the two-way fixed-effects coefficient uses already-treated units as controls and is biased under heterogeneous, time-varying effects. Estimates a clean 2x2 ATT(g, t) for every adoption cohort and period against not-yet-treated (or never-treated) units; `covariates` switches the cell estimator to outcome regression on the comparison units. Standard errors come from a unit-level bootstrap, with simultaneous bands from the bootstrapped maximum t-statistic. Returns a named list of tibbles: `att_gt` (per cell, with pointwise `conf.low`/`conf.high` and simultaneous `band.low`/`band.high`), `dynamic` (event study by time since adoption), `group`, `calendar` and `overall`.

### `kk_its(data, outcome, time, intervention_time, offset = NULL, transition = 0, family = c("gaussian","poisson","quasipoisson"), harmonic = 0, period = NULL, nw_lag = NULL, conf.level = 0.95)`
Interrupted time series by segmented regression, separating the immediate `level_change` from the `trend_change`. Standard errors are Newey-West by default (serial correlation is the standard failure mode of ITS); `harmonic`/`period` add Fourier seasonality, `offset` turns a count model into a rate model, `transition` drops a wash-out window. Returns a tibble of model terms with `estimate`, CI and `p.value` (plus the exponentiated `ratio` for count families). Attributes: `counterfactual` (per-period observed, fitted and extrapolated counterfactual with an interval, ready to plot), `impact` (final-period absolute and relative effect, cumulative difference with an interval) and `diagnostics` (Durbin-Watson, Ljung-Box).

### `kk_rdd(data, outcome, running, cutoff = 0, treatment = NULL, bandwidth = NULL, kernel = c("triangular","uniform","epanechnikov"), degree = 1, covariates = NULL, vcov_type = "HC1", bins = 20, conf.level = 0.95)`
Sharp and fuzzy regression discontinuity by local polynomial regression with kernel weights. Leave `treatment` `NULL` for a sharp design; supplying the actual treatment column makes it fuzzy, estimated by two-stage least squares with the cutoff indicator as the instrument (the estimand is then the complier effect at the cutoff). `bandwidth = NULL` selects the MSE-optimal bandwidth of Imbens-Kalyanaraman (2012). Returns a one-row tibble with `estimate`, `std.error`, CI, `p.value`, `bandwidth` and the effective sample size on each side; fuzzy designs add `first_stage` and `reduced_form`. Attributes: `bw_sensitivity` (the estimate at 0.5x-2x the bandwidth) and `binned` (binned means for plotting).

### `kk_rkd(data, outcome, running, cutoff = 0, treatment = NULL, bandwidth = NULL, kernel = "triangular", degree = 1, covariates = NULL, vcov_type = "HC1", conf.level = 0.95)`
Regression kink design: identifies the effect from a change in *slope* at the threshold rather than a jump in level, as when a policy formula bends at an income or size limit. With `treatment` supplied the estimate is the fuzzy kink — the change in the outcome slope divided by the change in the treatment slope. Kink designs need more data than discontinuity designs and are more sensitive to the polynomial order; supplying your own `bandwidth` is strongly preferred over the `n^(-1/7)` rule of thumb. Returns a one-row tibble; the bandwidth sensitivity table is attached as attribute `bw_sensitivity`.

### `kk_rd_density(data, running, cutoff = 0, bin_width = NULL, bandwidth = NULL, conf.level = 0.95)`
McCrary (2008) manipulation test: the log discontinuity in the density of the running variable at the cutoff. A non-significant result is a precondition for reading a `kk_rdd()` estimate causally. Returns a one-row tibble with the log density jump, its `std.error`, CI, `p.value` and the density on each side; the binned density is attached as attribute `density`.

### `kk_synth(data, outcome, unit, time, treated_unit, intervention_time, predictors = NULL, donors = NULL, placebo = TRUE, placebo_trim = Inf)`
Synthetic control for a single treated unit: a non-negative, sum-to-one weighted average of donor units fitted to the pre-intervention period, so the comparator interpolates rather than extrapolates. Weights are found by projected-gradient minimisation over the simplex on the pre-period outcomes plus any `predictors` (row-standardised). Inference is a placebo permutation over the donor pool, so the smallest attainable p-value is `1/(1 + n_donors)`. Returns a one-row tibble with `att`, `att_pct`, `pre_rmspe`, `post_rmspe`, `rmspe_ratio` and `p.value`. Attributes: `weights`, `path` (per-period observed, synthetic and gap series) and `placebo`.

### `kk_iv(data, outcome, treatment, instruments, covariates = NULL, cluster = NULL, vcov_type = "HC1", ar_ci = TRUE, conf.level = 0.95)`
Instrumental-variable regression by two-stage least squares; the estimand is the local average treatment effect on compliers. Returns a one-row tibble with `estimate`, `std.error`, CI, `p.value`, the `ols` estimate for comparison, `first_stage_f`, a `weak_instrument` flag, and the Anderson-Rubin weak-instrument-robust interval (`ar.low`, `ar.high`), which stays valid however weak the instrument. Attributes: `first_stage` and `diagnostics` (first-stage F, Sargan over-identification test when over-identified, Wu-Hausman endogeneity test).

### `kk_ancova(data, outcome, treatment, baseline, covariates = NULL, vcov_type = "HC3", conf.level = 0.95)`
Pretest-posttest ANCOVA: the follow-up outcome regressed on treatment and the baseline value of the same measure. More efficient than either the comparison of follow-up means or the comparison of change scores, and robust to regression to the mean. Returns a one-row tibble with the adjusted `estimate`, CI, `p.value`, the standardised adjusted group means `ey1`/`ey0`, and the `change_score` and `posttest_only` estimates for comparison.
