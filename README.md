...
A comprehensive R package designed for medical statistics, epidemiology, and general clinical data analysis. It provides a suite of tidyverse-friendly tools that take individual-level data as their first argument, support piping (`%>%` / `|>`), and return tidy tibbles.

**Grouped input.** The analysis functions honour `dplyr::group_by()`: pipe a grouped
data frame in and the analysis is run once per group, with the grouping columns
prefixed to the result. This is genuinely stratified — not a pooled estimate:
...
Plotting helpers and column-wise utilities (`kkonehot()`, `mutate_round()`,
`format_tibble()`) are group-agnostic by design.
...
Attach whatever else your script uses, as usual:
...
`library(kkstatfun)` silently attached the whole tidyverse. That also meant
`kkstatfun::kk_coxph(...)` could fail in a clean session, because the `::` form
loads the namespace without attaching anything. Both are fixed.
...
Computes **time-varying Absolute Risk Reduction (ARR)** and **Number Needed to Treat (NNT)** or Harm (NNH) from Kaplan-Meier survival curves at specified time points, with Greenwood-based confidence intervals.

*   **Clinical Example**: Assessing the time-varying NNT for survival benefit from a new oncology therapy across 10, 20, and 30 time units of follow-up.
...
> **Interpretation.** At 20 units of follow-up, survival is 73.8% in treated vs 45.7% in controls ($\Delta = 28.1\%$). The time-varying NNT is 3.56 (95% CI 2.40–6.90), indicating ~4 patients need treatment to prevent 1 mortality event by time 20.
...
Calculates **directly age-standardized rates with exact Gamma confidence intervals** (Tiwari et al. 2006 / Fay-Feuer 1997 NCI SEER standard), overcoming undercoverage of normal CIs when age-specific event counts are small or zero.

*   **Epidemiological Example**: Computing age-adjusted rare cancer incidence rates with exact SEER modified gamma confidence intervals.
...
> **Interpretation.** Crude rate is 222.7 per 100,000; directly age-adjusted rate is 235.1 per 100,000. Tiwari's exact modified gamma 95% CI is 201.2 to 273.7 per 100,000, providing exact coverage even with sparse counts in young age groups.
...
The modelling functions (`kk_reg()`, `kk_coxph()`, `kk_rr_reg()`, `kk_rate_reg()`, `kk_firth()`) keep the fitted model alongside the tidy coefficient table. `kk_model()` retrieves it, so the whole downstream modelling ecosystem — above all **`emmeans`** — is available without refitting anything.

*   **Clinical Example**: Reporting adjusted (model-based) mean systolic BP per treatment arm, and the contrast between arms, from the same model that produced the coefficient table.
...
> **Interpretation.** `kk_emmeans()` reports the **model-adjusted** mean outcome per group — the mean each arm would have at common covariate values — which is what belongs in a results table, rather than the raw group means. The `pairwise` contrast is the adjusted between-arm difference with its CI and p-value: 9.5 mmHg lower on treatment (95% CI 7.3–11.8), comfortably covering the 8 mmHg effect built into the simulated cohort. Confidence limits are always named `conf.low`/`conf.high`, smoothing over the fact that `emmeans` itself switches between `lower.CL` and `asymp.LCL` depending on the model.

For anything beyond means and contrasts, take the model and use `emmeans` directly:
...
> **Interpretation.** `kk_model()` returns an ordinary `lm`/`glm`/`coxph`, so every `emmeans` verb works unchanged — as do `anova()`, `plot()`, `predict()` and packages such as `marginaleffects`. Use `kk_model(fit, "univariate", "age")` for a single-predictor model, and on grouped input `kk_model(fit, group = "A")` for one group's model. `kk_model_data(fit)` returns the fitting data if `emmeans` ever needs it passed explicitly.
...
Fits continuous exposure-outcome associations using **Restricted Cubic Splines (RCS)**, returning predicted Odds Ratios, Risk Ratios, or linear effects relative to a reference exposure level $x_{\text{ref}}$, along with formal Likelihood Ratio Tests for non-linearity.

*   **Epidemiological Example**: Evaluating non-linear dose-response curves for BMI and cardiovascular risk relative to a reference BMI of 22 $\text{kg/m}^2$.
...
> **Interpretation.** Predicts odds ratios across a 100-point exposure grid relative to reference BMI = 22. The non-linearity attribute (`attr(res, "nonlinearity_test")`) contains the LRT comparing spline vs linear model ($p_{\text{nonlinearity}}$).
...
#### `space = "oklch"` — perceptually uniform schemes

HSV treats every hue at a given "value" as equally light, but the eye does not: a pure yellow at `V = 0.84` looks far brighter than a pure blue at the same value. Rotating hue in HSV therefore produces palettes whose colours shout at different volumes. `space = "oklch"` builds the same schemes in **OKLCh**, where lightness is perceptual, so a hue rotation holds apparent lightness fixed. Colours that fall outside sRGB are brought back by reducing chroma rather than by clipping the RGB channels, which would shift the hue.
...
<div class="figure">
<img src="man/figures/README-ex-oklch-1.png" alt="plot of chunk ex-oklch" width="75%" />
<p class="caption">plot of chunk ex-oklch</p>
</div>

> **Interpretation.** The HSV triadic answers `#D62828` with a fluorescent `#28D628`; the OKLCh one gives `#008F33`, which carries the same visual weight as the seed. The default stays `space = "hsv"` so palettes in existing scripts and published figures do not change — use `"oklch"` for new work. `set_plot_colors()` accepts the same argument: `set_plot_colors("#D62828", scheme = "triadic", space = "oklch")`.

---

### 11. Figure Accessibility & Colour Vision

Roughly **8% of men and 0.5% of women** have a red–green colour vision deficiency, and journals increasingly require figures that survive greyscale printing. These functions check a palette *before* it reaches a manuscript.

#### `kk_show_cvd(colors)` / `kk_cvd(colors, type, severity)`

`kk_cvd()` renders colours as they are seen with deuteranopia, protanopia, tritanopia or achromatopsia, using the physiologically-based transforms of Machado, Oliveira & Fernandes (2009) applied in linear-light RGB. `severity` runs from 0 (normal trichromacy) to 1 (full dichromacy); intermediate values give *anomalous trichromacy*, the common mild form. `kk_show_cvd()` draws the result as a swatch grid.


``` r
kk_cvd(c("#D62828", "#003049", "#F77F00"), type = "deutan")
#> # A tibble: 3 × 5
#>   color   hex     vision severity simulated
#>   <chr>   <chr>   <fct>     <dbl> <chr>    
...
<div class="figure">
<img src="man/figures/README-ex-cvd-1.png" alt="plot of chunk ex-cvd" width="75%" />
<p class="caption">plot of chunk ex-cvd</p>
</div>

> **Interpretation.** The flag palette's red (`#D62828`) and amber (`#F77F00`) both collapse towards olive for deuteranopes and protanopes — a grouped bar chart using those two colours is unreadable for about one man in twelve. Blue survives every deficiency, which is why blue–orange rather than red–green is the safe two-colour contrast.

#### `kk_pal_check(colors, background)`

Audits a whole palette at once. For every colour and every vision type it reports the perceptual distance (Euclidean distance in OKLab, "delta-E OK") to the **nearest other colour in the palette** as that reader sees it, plus the WCAG contrast against the plotting background. An empty result from the filter below means the palette passes.


``` r
chk <- kk_pal_check(c("#D62828", "#003049", "#F77F00"), background = "#FFFFFF")

# anything that fails: colours too close, or too faint against white
chk |> filter(!distinct | !graphic)
#> # A tibble: 4 × 8
...
> **Interpretation.** `min_dist` below `min_dist =` (default 0.10) means two categories will read as one colour; `graphic = FALSE` means the colour falls under the 3:1 WCAG threshold for lines, points and other non-text objects. Here amber on white is the weak link at 2.6:1. Fix either by changing the palette or — often better in a scientific figure — by adding a redundant non-colour cue: `linetype`, point `shape`, or direct labels.

#### `kk_pal_safe(n, seed_colors)`

Builds a categorical palette whose colours stay far apart in OKLab **simultaneously** for normal, deuteranopic, protanopic and tritanopic vision, and that clear the WCAG non-text contrast threshold against the background. Selection is deterministic farthest-point sampling, so the same call always returns the same palette — figures stay reproducible. Anchor it on an institutional colour with `seed_colors`.


``` r
pal <- kk_pal_safe(5, seed_colors = "#003049")
pal
...
<div class="figure">
<img src="man/figures/README-ex-palsafe-1.png" alt="plot of chunk ex-palsafe" width="75%" />
<p class="caption">plot of chunk ex-palsafe</p>
</div>

> **Caveat.** `"achroma"` (greyscale) is deliberately **not** in the default `type`. Including it forces the categories to separate on lightness alone, which caps a usable palette at three or four colours. If the figure must survive photocopying, either ask for fewer categories with `type = c("normal", "deutan", "protan", "tritan", "achroma")` or keep more colours and add a non-colour cue.

#### `kk_contrast(colors, background)` / `kk_color_convert(colors)`

`kk_contrast()` gives WCAG 2.1 contrast ratios with the AA/AAA conformance flags — 4.5:1 for body text, 3:1 for large text and graphical objects, 7:1 for AAA. `kk_color_convert()` exposes the underlying OKLab/OKLCh coordinates and relative luminance.


``` r
kk_contrast(c("#D62828", "#F77F00", "#003049"), background = "#FFFFFF")
#> # A tibble: 3 × 8
#>   color   background luminance contrast aa_text aa_large aaa_text graphic
...
#> # A tibble: 3 × 11
#>   color   hex       red green  blue  ok_l    ok_a    ok_b ok_chroma ok_hue luminance
#>   <chr>   <chr>   <int> <int> <int> <dbl>   <dbl>   <dbl>     <dbl>  <dbl>     <dbl>
#> 1 #D62828 #D62828   214    40    40 0.568  0.186   0.0948    0.208    27.1    0.160 
#> 2 #003049 #003049     0    48    73 0.294 -0.0343 -0.0559    0.0656  238.     0.0259
#> 3 #F77F00 #F77F00   247   127     0 0.719  0.104   0.146     0.179    54.4    0.350
```

> **Interpretation.** Use `kk_contrast()` for annotation text and thin lines drawn over a panel background, and `ok_l` from `kk_color_convert()` when you need to know whether two colours differ in lightness as well as hue — a difference that survives both colour blindness and greyscale printing.

> **Sources.** OKLab: Ottosson (2020). CVD transforms: Machado, Oliveira & Fernandes, *IEEE TVCG* 15(6), 1291–1298 (2009) — output verified identical to `colorspace::deutan()`. Contrast: W3C WCAG 2.1, SC 1.4.3 and 1.4.11. Implemented with base `grDevices` only, so no extra dependencies.

---

### 12. Data Utilities & Setup
...
### 13. Health Economics & HTA (Cost-Effectiveness)
...
### 14. Infectious Disease Transmission Modeling
...
### 15. Advanced Methodological Suite
