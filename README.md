...
> **Interpretation.** The output table presents both crude and adjusted attributable fractions. Bruzzi's model-standardization (`PAF (Adjusted - Bruzzi)`) controls for confounding by age and BMI to quantify the proportion of disease cases in the population that would be prevented if exposure were entirely eliminated.
...
> **Interpretation.** The output table reports Subdistribution Hazard Ratios ($\text{SHR} = e^\beta$) for both univariate and multivariable models. Unlike standard Cox models that censor competing events (which can bias cumulative incidence interpretation), Fine-Gray regression explicitly keeps individuals who experience competing events in the risk set with appropriate weights, directly modeling the cumulative incidence function over time.
